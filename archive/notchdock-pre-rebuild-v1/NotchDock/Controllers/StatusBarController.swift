...
final class StatusBarController: NSObject {
    ...
    private func openMenu() {
        ...
        if viewModel.enableWorkspace {
            let workspaceItem = NSMenuItem(title: "Toggle Workspace", action: #selector(toggleWorkspaceMenuAction), keyEquivalent: "")
            workspaceItem.target = self
            menu.addItem(workspaceItem)
        } else {
            let workspaceDisabled = NSMenuItem(title: "Workspace: disabled (Settings)", action: nil, keyEquivalent: "")
            workspaceDisabled.isEnabled = false
            menu.addItem(workspaceDisabled)
        }
