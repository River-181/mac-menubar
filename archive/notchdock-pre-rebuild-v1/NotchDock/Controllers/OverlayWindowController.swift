...
final class OverlayWindowController {
    ...
    init(viewModel: NotchDockViewModel, geometry: NotchGeometryCalculating = NotchGeometryCalculator()) {
        ...
        let initialSize = viewModel.overlayState.panelFrameSize
        let panel = NSPanel(
    ...
    func layoutPanel() {
        ...
        let frame = geometry.capsuleFrame(
            screen: screen,
            visualState: viewModel.overlayState,
    ...
    private func sampleTopTrigger() {
        ...
        if dragActive, rawInsideTrigger, (viewModel.overlayState == .idle || viewModel.overlayState == .peek) {
            viewModel.prepareDropPreview()
        }

        setPanelInteractionEnabled(insideCapsule || (dragActive && rawInsideTrigger))
