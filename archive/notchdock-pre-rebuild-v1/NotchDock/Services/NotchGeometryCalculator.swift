...
final class NotchGeometryCalculator: NotchGeometryCalculating {
    ...
    func triggerZone(screen: NSScreen) -> CGRect {
        ...
        let notchWidth = max(0, frame.width - snapshot.safeLeft - snapshot.safeRight)
        let effectiveWidth = snapshot.hasNotch
            ? Self.clamp(notchWidth + 28, min: 170, max: 250)
            : triggerWidth
        let effectiveHeight: CGFloat = snapshot.hasNotch ? 28 : 22
        return CGRect(
