...
final class NotchDockViewModel: ObservableObject {
    ...
    @Published var enableWorkspace = false
    ...
    private let workspaceKey = "notchdock.workspace.v1"
    ...
    func transition(_ event: OverlayEvent) {
        ...
        case .stage2:
            guard enableWorkspace else { break }
    ...
    func ingestPointerSample(
        ...
        processTriggerState(rawInside: isTriggerRawInside, timestamp: sample.timestamp, isDragging: isDragging)
        if isDragging {
            beginDragSession()
    ...
    func toggleWorkspace(trigger: Stage2Trigger = .hotkey) {
        guard enableWorkspace else {
            showToast("Workspace is disabled in settings.", isError: true)
            return
        }
    ...
    func setWorkspaceEnabled(_ enabled: Bool) {
        ...
        defaults.set(enabled, forKey: workspaceKey)
        if !enabled, overlayState == .workspace {
            animateStateChange(to: .expand)
            recomputeArrangement()
        }
    }
    ...
    private func scheduleDragStage2IfNeeded() {
        cancelDragStage2()
        guard enableWorkspace else { return }
    ...
    private func loadPreferences() {
        ...
        enableWorkspace = defaults.object(forKey: workspaceKey) as? Bool ?? false
    ...
    private func processTriggerState(rawInside: Bool, timestamp: TimeInterval, isDragging: Bool) {
        let enterDelay = isDragging ? 0.015 : triggerEnterDelay
        let exitDelay = isDragging ? 0.08 : triggerExitDelay
        switch triggerState {
        case .outside:
            if rawInside {
